...
import AdminDashboardContent from "@/app/components/AdminDashboardContent";
...
// Configuration for Mahasiswa View (Plain objects, components handled inside loop if needed)
...
export default async function DashboardPage() {
    ...
    // ──────────────────────────────────────────────────
    if (isAdmin) {
        const allAssetCategories = await prisma.assetCategory.findMany({
            include: {
                items: {
                    select: {
                        id: true,
        ...
        const categoriesForManagement = allAssetCategories.map(category => {
        ...
        const pendingBorrowings = await prisma.borrowing.findMany({
            ...
            include: {
                ...
                assetItem: {
                    include: {
                        category: { select: { nama: true, prefix: true } },
                    },
                },
            },
            ...
            <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
                <div className="mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-foreground sm:text-3xl">Dashboard Admin</h1>
                            <p className="text-sm text-muted">Kelola peminjaman alat & booking ruangan laboratorium</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="mb-8 grid grid-cols-1 gap-4 sm:grid-cols-4">
                    ...
                    <div className="rounded-2xl border border-border bg-surface p-5 shadow-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-2xl font-bold text-foreground capitalize">{session.user.role.toLowerCase().replace("_", " ")}</p>
                ...
                <AdminDashboardContent 
                    ...
                    categoriesForManagement={categoriesForManagement}
        ...
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
            <div className="mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-foreground sm:text-3xl">Dashboard Saya</h1>
                        <p className="text-sm text-muted">Halo, {session.user.nama} — Berikut riwayat aktivitas Anda.</p>
                    </div>
                </div>
            </div>

            <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
                {(["PENDING", "DIPINJAM", "SELESAI", "DITOLAK"] as const).map((status) => {
                                ...
                                {borrowings.map((b, idx) => {
                                    ...
                                    return (
                                        <tr key={b.id} className="transition-colors hover:bg-surface-hover/30">
                                            ...
                                            <td className="px-6 py-4 text-sm font-medium text-foreground">{b.assetItem.category.nama}</td>
                                            <td className="px-6 py-4 font-mono text-xs text-muted">{b.assetItem.category.prefix}-{b.assetItem.nomorUrut}</td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm text-muted">
                                                {new Date(b.tglPinjam).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })}
                                            </td>
                                            <td className="whitespace-nowrap px-6 py-4 text-sm text-muted">
                                                {b.returnedDate ? new Date(b.returnedDate).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" }) : "—"}
                                            </td>
